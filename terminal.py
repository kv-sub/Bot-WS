...
def run_command_in_new_terminal(command):
    ...
    subprocess.Popen(['gnome-terminal', '--', 'bash', '-c', f'source ~/bot_ws/install/setup.bash && {command}; exec bash'])
...
def execute_gazebo_simulation():
    for cmd in commands["gazebo_simulation"]:
        run_command_in_new_terminal(cmd)
...
# Example of executing commands in new terminals
